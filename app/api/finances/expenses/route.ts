  ...
  try {
    ...
    if (!type || type === "utilities") {
      const [items, count] = await Promise.all([
        prisma.utility.findMany({ where: projectWhere, skip: type ? skip : 0, take: type ? limit : 50, orderBy: { usageDate: "desc" }, include: { project: { select: { name: true } }, receipt: { select: { fileUrl: true } } } }),
    ...
    if (!type || type === "charges") {
      const [items, count] = await Promise.all([
        prisma.siteCharge.findMany({ where: projectWhere, skip: type ? skip : 0, take: type ? limit : 50, orderBy: { chargeDate: "desc" }, include: { project: { select: { name: true } }, receipt: { select: { fileUrl: true } } } }),
    ...
    if (!type || type === "other") {
      const [items, count] = await Promise.all([
        prisma.otherExpense.findMany({ where: projectWhere, skip: type ? skip : 0, take: type ? limit : 50, orderBy: { expenseDate: "desc" }, include: { project: { select: { name: true } }, receipt: { select: { fileUrl: true } } } }),
...
export async function POST(req: NextRequest) {
  try {
    ...
    // All three roles can add expenses (accountant can add office expenses separately)
    let type: string, projectId: string, name: string, category: string;
    let amount: number, paymentMethod: string, date: string, description: string;
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      ({ type, projectId, name, category, amount, paymentMethod, date, description } = body);
    }
    ...
    // Check project access for site managers
    if (session.user.role === "SITE_MANAGER") {
    ...
    if (receiptFile && receiptFile.size > 0) {
      try {
        ...
        const receipt = await prisma.receipt.create({
          data: { fileUrl, fileName: receiptFile.name, fileType: receiptFile.type, projectId },
        });
    ...
    if (type === "utility") {
      record = await prisma.utility.create({
        data: { projectId, name, category: category || "OTHER", amount, paymentMethod: paymentMethod as any, usageDate: new Date(date), description, receiptId },
      });
    } else if (type === "charge") {
      record = await prisma.siteCharge.create({
        data: { projectId, name, category: category || "OTHER", amount, paymentMethod: paymentMethod as any, chargeDate: new Date(date), description, receiptId },
      });
    } else {
