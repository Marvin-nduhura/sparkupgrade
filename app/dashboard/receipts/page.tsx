...
import {
  Receipt, Search, Eye, Download, ChevronLeft, ChevronRight,
  FileText, Zap, AlertCircle, CreditCard, Package, Trash2
} from "lucide-react";
...
export default function ReceiptsPage() {
  ...
  const canDelete = session?.user?.role === "SYSTEM_ADMIN" || session?.user?.role === "ACCOUNTANT";
  ...
  return (
    <div className="page-container">
          ...
          {TYPE_TABS.map(t => (
            <button key={t.value} onClick={() => { setType(t.value); setPage(1); }}
              className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-all",
                type === t.value ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {receipts.map((rec: any, i: number) => {
            ...
            const typeBadgeColor =
              rec._source === "purchase" ? "bg-blue-100 text-blue-700" :
            ...
            return (
              <motion.div key={rec.id}
                ...
                <div
                  ...
                  onClick={() => { setViewUrl(rec.fileUrl); setViewFileName(rec.fileName); }}>
                  {isImage ? (
                    ...
                    <div className="flex flex-col items-center gap-2 p-3">
                      ...
                      <p className="text-[10px] text-muted-foreground text-center break-all">
                        {rec.fileType?.includes("pdf") ? "PDF" : rec.fileType?.includes("word") ? "DOC" : rec.fileType?.includes("excel") ? "XLS" : "FILE"}
                      </p>
                    </div>
                  )}
                  {rec.aiVerified && (
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1.5">
                    <button onClick={e => { e.stopPropagation(); setViewUrl(rec.fileUrl); setViewFileName(rec.fileName); }}
                      className="p-2 bg-white/90 rounded-xl text-gray-800 hover:bg-white transition-colors">
                      <Eye className="w-4 h-4" />
                    </button>
                    <button onClick={e => { e.stopPropagation(); downloadReceipt(rec.fileUrl, rec.fileName); }}
                      className="p-2 bg-white/90 rounded-xl text-gray-800 hover:bg-white transition-colors">
                      <Download className="w-4 h-4" />
                    </button>
                    {canDelete && (
                      <button onClick={e => handleDelete(e, rec.id, rec.fileName)}
                        className="p-2 bg-red-500/90 rounded-xl text-white hover:bg-red-600 transition-colors">
      ...
      {viewUrl && <ReceiptViewModal url={viewUrl} fileName={viewFileName} onClose={() => { setViewUrl(""); setViewFileName(""); }} />}
    </div>
  );
}
